...
export function CallTrendsChart({ data }: any) {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  return (
    <Card className={isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'}>
      <CardHeader>
        <CardTitle className={isDark ? 'text-white' : 'text-gray-900'}>Call Trends</CardTitle>
      </CardHeader>
      <CardContent>
        <div className={`h-64 flex items-center justify-center rounded-lg border ${
          ...
          {data ? `${data.length} data points` : 'Call trends chart'}
        </div>
      </CardContent>
    </Card>
  );
