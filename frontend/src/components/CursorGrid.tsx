  ...
  useEffect(() => {
    const wake = () => {
      ...
      if (p.gridOpacity > 0) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        for (let cCol = 0; cCol <= Math.ceil(canvas.width / p.cellSize) + 1; cCol++) {
          const x = Math.round(cCol * p.cellSize) + 0.5;
          ctx.moveTo(x, 0);
          ctx.lineTo(x, canvas.height);
        }
        for (let cRow = 0; cRow <= Math.ceil(canvas.height / p.cellSize) + 1; cRow++) {
          const y = Math.round(cRow * p.cellSize) + 0.5;
          ctx.moveTo(0, y);
